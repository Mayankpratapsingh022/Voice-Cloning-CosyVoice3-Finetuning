...
Runs on the RunPod pod, not locally.
...
def generate_holdout_samples(speaker_prefix: str, experiment_name: str) -> str:
    ...
    Returns the directory the generated WAVs were written to.
    """
    ...
    holdout_dir = RemotePaths.DATASET_DIR / speaker_prefix / "holdout"
    out_dir = RemotePaths.CHECKPOINT_DIR / speaker_prefix / experiment_name / "eval_samples"
...
def evaluate_experiment(speaker_prefix: str, experiment_name: str) -> list[EvalResult]:
    ...
    samples_dir = RemotePaths.CHECKPOINT_DIR / speaker_prefix / experiment_name / "eval_samples"

    results: list[EvalResult] = []
    for utt_id, reference_text in holdout_text.items():
        generated_path = samples_dir / f"{utt_id}_0.wav"
        if not generated_path.exists():
            logger.warning("no generated sample for %s, skipping (did generate_holdout_samples run first?)", utt_id)
            continue
...
def evaluate_sweep(speaker_prefix: str, experiment_names: list[str]) -> str:
    """Generate + score every experiment in the sweep, write the comparison report.

    Returns the path to the markdown summary.
    """
    for name in experiment_names:
        generate_holdout_samples(speaker_prefix, name)

    all_results: list[EvalResult] = []
    for name in experiment_names:
        all_results.extend(evaluate_experiment(speaker_prefix, name))

    out_dir = RemotePaths.CHECKPOINT_DIR / speaker_prefix / "eval_report"
